...
## 2026-06-12 — rule/verification

**Status:** applied — 2026-06-12 — ~/.claude/MANDATORY.md #4 + commands/ship.md step 6

**Learning:** A whole CLASS of bugs lives in the GAP between what the test harness exercises and what the real client does — and green CI is structurally blind to all of them. Three forms seen on FocusBoard, each invisible to a full passing suite, each caught only against the DEPLOYED artifact: (1) a runtime ADAPTER the tests bypass — route tests call `app.fetch()`/`app.request()` in-process, so the Vercel `(req,res)→fetch` adapter is never exercised (the original 504 = a Web handler on the Node runtime; later, the adapter re-serializing form bodies as JSON so every OAuth POST parsed to empty fields); (2) a BROWSER-enforced policy the tests can't enforce — CSP is only a response-header STRING server-side; a Node test can assert the header VALUE but never its EFFECT (blocking a form submit / script / frame / redirect) because there is no user agent to obey it. FocusBoard's `form-action 'self'` was present and "correct" yet silently blocked the entire OAuth login, because CSP form-action polices the whole form-initiated navigation including the server's cross-origin 302. The mitigation already exists and must be USED, not just present: the runtime-smoke gate hits the live artifact AND supports a Playwright UI rung (`SHIPIT_SMOKE_UI`) — for any security-sensitive browser surface (an auth/login/redirect page, anything that sets CSP/headers that change navigation), the UI rung should actually LOAD it in a real browser and exercise the critical action (submit/redirect), because that is the only thing that experiences CSP. A unit test can only pin the header string (regression guard), never the behaviour.

**Why:** FocusBoard Phase 6.2 (hosted MCP + OAuth). Each of the three bugs passed build + typecheck + every route test + cross-model review; each surfaced only in a real deployed browser (the OAuth one: Claire's actual Cowork connect, browser console "violates form-action 'self'"). Sharpens MANDATORY #4 (green CI ≠ runtime works) with the SPECIFIC mechanism: name the two harness-divergence axes (the adapter the tests bypass; browser-enforced policy the tests can't run) so the next auth/redirect/header feature gets a real-browser smoke rung by default instead of trusting header-string assertions.

**Apply to:** `~/.claude/MANDATORY.md (sharpen #4 with the adapter + browser-policy axes) + shipit-v4 commands/ship.md step 6 (runtime-smoke: when a PR touches a CSP/header/auth/redirect BROWSER surface, the Playwright UI rung must load that page and exercise submit/redirect, not just assert headers)`.

**Ready-to-apply snippet:**

```
- **Bugs live in the gap between what the harness runs and what the real client does — name the two axes.** (a) The runtime ADAPTER the tests bypass: route tests call app.fetch()/app.request() in-process, so the Vercel (req,res)→fetch bridge is never exercised (the 504 Web-handler bug; the form-body-parsed-as-JSON bug — every OAuth POST got empty fields). (b) BROWSER-enforced policy the tests can't run: CSP is just a response header server-side; a Node test asserts the STRING, never the EFFECT — FocusBoard's `form-action 'self'` was present and correct yet blocked the whole OAuth login, because form-action polices the form's cross-origin 302 redirect. Mitigation: for any auth/login/redirect/CSP browser surface, the runtime-smoke Playwright rung must LOAD the page and exercise submit/redirect in a real browser; header-string assertions are a regression pin, not proof it works.
```

## 2026-06-13 — tooling/enforcement

**Status:** applied — 2026-06-13 — gates/check-docs-coverage.sh + gates/ci-templates/docs-check.yml + install-gates.sh (FocusBoard = first customer: .shipit-gates/check-docs-coverage.sh + .github/workflows/docs-check.yml)

**Learning:** A docs-sync gate that only checks "code changed → SOME doc changed" is a TOUCH gate, and a touch gate gives FALSE CONFIDENCE: it is satisfied by editing ANY doc (HISTORY.md, an unrelated docs/FOO.md), so a PR can add 10 API routes / a new table and edit HISTORY.md and PASS while docs/API.md and docs/SUPABASE.md silently go stale. The fix is a CONTENT-COVERAGE gate (`check-docs-coverage.sh`): extract the authoritative manifest of NAMES from source — API route keys from the `ROUTE_SCOPES` map and table/function names from `supabase/migrations/*.sql` — and assert each name appears as a literal substring in its designated doc (`docs/API.md` / `docs/SUPABASE.md`), failing with the exact missing names. It is pure grep/sed (zero token cost, runs in seconds), config-driven (`SHIPIT_ROUTES_SRC/DOC/RE`, `SHIPIT_DB_GLOB/DOC`), honours the same `[no-docs]` override, and auto-SKIPS on repos lacking those source paths so it generalises safely. Ship it as a SIBLING job to the touch gate in `docs-check.yml` (keep both — touch catches "you forgot to update narrative docs", coverage catches "the reference docs are wrong"), wire it into `install-gates.sh` (fresh installs get it via `cp gates/*.sh` + `ci-templates/*.yml`; note that `--update` does NOT add a new gate's CI job to an existing repo — that needs a full re-install or a hand-added job).

**Why:** FocusBoard Phases 3–6 (focus sessions, card mutation + cards mirror, agent workflows, hosted MCP + OAuth) ALL shipped through the gated workflow with the "docs-check" gate GREEN, yet docs/API.md documented ~4 of ~35 routes (3 of those wrong) and docs/SUPABASE.md covered ~2 of ~10 schema objects (api_tokens, focus_sessions, mcp_confirmations, all 4 oauth_* tables, and all 3 functions fb_add_card/fb_mutate_card/sync_cards_from_app_state were entirely undocumented). The gate did not catch it because it only verifies THAT a doc changed, never WHAT it says — exactly the false-confidence class the check-docs-sync.sh header already warns about for a different reason (a no-op match), now generalised to "matched the wrong doc". Run against FocusBoard's drifted tree the new gate flags all 35 missing routes + all 10 missing DB objects and exits 1. Harness-level flaw (focusboard's gate was byte-identical to the canonical), so the fix lands in shipit-v4's gates/ + ci-templates/ to propagate to every future project, with FocusBoard as the first customer.

**Apply to:** `shipit-v4: gates/check-docs-coverage.sh (new) + gates/ci-templates/docs-check.yml (add docs-coverage job) + scripts/install-gates.sh (header + --update note). FocusBoard: .shipit-gates/check-docs-coverage.sh + .github/workflows/docs-check.yml docs-coverage job. (Separately owed: backfill docs/API.md + docs/SUPABASE.md to make the gate green — the gate is the wall; the backfill is a content task for @docs/build engineer.)`

**Ready-to-apply snippet:**

```
- **Docs-sync must be a CONTENT gate, not a TOUCH gate.** "code changed → SOME doc changed" passes when you edit ANY doc, so reference docs (docs/API.md, docs/SUPABASE.md) drift silently while the gate stays green (FocusBoard Phases 3–6: ~4 of ~35 routes and ~2 of ~10 schema objects documented, every PR green). Ship check-docs-coverage.sh as a SIBLING to the touch gate: extract route keys from ROUTE_SCOPES + table/function names from supabase/migrations/*.sql, assert each is a literal substring of its designated doc, fail listing the missing names. Pure grep/sed (no token cost), config-driven, [no-docs] override, auto-skips repos without those paths. Keep BOTH gates (touch = "you forgot narrative docs"; coverage = "the reference docs are wrong"). Wire into install-gates.sh; note --update won't add a new gate's CI job to existing repos.
```
