...
- **Global guards** are wired in `hooks/hooks.json`. **Per-repo gates** are wired by the installer `scripts/install-gates.sh` (S5; copies `gates/` → `.shipit-gates/`, drops CI + a `pre-push` hook). Installed gates are **frozen copies** — a plugin-side gate fix does NOT reach them automatically; `scripts/install-gates.sh <repo> --update` refreshes only the `.shipit-gates/` scripts the repo **already has** (so a targeted install keeps its shape; CI / `settings.json` / `smoke.conf` untouched) to propagate a fix. **`/ship`** (`commands/ship.md`) runs the lot on demand, but the hooks fire even when it isn't called — that's the v3 fix.
