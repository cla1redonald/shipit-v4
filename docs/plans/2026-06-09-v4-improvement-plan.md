...
- 🟡 **P5 in progress** — validation & loop-closing (spec below). **This is the gate before any P6/new feature.** The base must survive ~10 real PRs across ≥3 repos AND the learning loop must be *proven* to fire (not assumed) before more machinery is stacked on.
  ...
  - 🟢 **P5a — install coverage met (3/3 repos):** **roami-hobbies** is customer #3 (PR #10, merged) — a Next.js App Router app with **no existing CI**, so it exercised the **full `install-gates.sh` happy path** (FocusBoard/ProveIt both forced targeted installs). `runtime-smoke` wired to `/,/api/waitlist` (POST-only, no LLM → GET=405, cost-safe); verified live on prod (`/`→200, `/api/waitlist`→405, `<main>` renders) and fired in CI. **Two more findings:** (3) the generic `docs-check` CODE_RE (`^(scripts|agents|commands|hooks|gates)/`) assumes ShipIt's OWN layout — it doesn't match an app repo's `src/`, so docs-check is **inert** on app repos (sibling of the ProveIt monorepo gap; installer needs a per-repo CODE_RE / layout-detect). (4) **Vercel preview-protection blind spot:** a protected preview deploy returns **401 on every route**, so the HTTP tier passes vacuously (401≠5xx) AND the **UI tier false-passed** (asserted `main` on the Vercel protection page, not the app). Harden: the UI tier should assert a non-401/403 status before trusting a render; the real signal is the *production* `deployment_status` event. *Still open:* the **~10 real PRs + battle-log** (observational, accrues over time) — install coverage is done; the *evidence over real PRs* is not.
  - ⏭️ **P5d** still open (review-earns-keep data now accruing on FocusBoard + ProveIt + roami-hobbies — tally over the next PRs).

### P5 findings backlog (from battle-testing — feed into a future P6)
1. **`install-gates.sh` assumes a root single-package repo** — doesn't fit a monorepo (ProveIt, app in `web/`). Needs a monorepo mode / targeted-install path.
2. **`docs-check` default CODE_RE assumes ShipIt's own dir layout** (`scripts|agents|commands|hooks|gates`) — inert on app repos using `src/`/`app/`/`web/`. Needs a per-repo CODE_RE or layout-detect.
3. **runtime-smoke UI tier can false-pass on a Vercel-protected preview** (401 → loads the protection page, which has `<main>`). Assert non-401/403 before trusting the render; lean on production `deployment_status`.
4. **`note-applied` self-report under-counts** (P5b honest limit) — the action-match tier mitigates for code-shaped rules; judgment rules remain lossy.
