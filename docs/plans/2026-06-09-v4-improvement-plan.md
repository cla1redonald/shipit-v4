...
- ⏭️ **P5 next** — validation & loop-closing (spec below). **This is the gate before any P6/new feature.** The base must survive ~10 real PRs across ≥3 repos AND the learning loop must be *proven* to fire (not assumed) before more machinery is stacked on.
