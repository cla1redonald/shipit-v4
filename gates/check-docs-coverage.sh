...
FUNCS_DIR="${SHIPIT_FUNCS_DIR:-api}"
...
# Vercel functions are files under $FUNCS_DIR that export a default handler but
# aren't the Hono router, a _lib helper, or a test. Their HTTP path is derived
# from the file path: api/ai/parse-card.ts → /api/ai/parse-card. These live
# OUTSIDE the ROUTE_SCOPES table, so without this manifest they can rot
# undocumented (exactly how the 5 api/ai/* endpoints slipped through).
if [ -d "$FUNCS_DIR" ]; then
  func_routes=$(grep -rlE 'export[[:space:]]+default' "$FUNCS_DIR" --include='*.ts' 2>/dev/null \
                  ...
                  | sed -E "s#^${FUNCS_DIR%/}/(.*)\.ts\$#/${FUNCS_DIR%/}/\1#" | sort -u)
