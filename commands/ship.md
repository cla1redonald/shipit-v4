...
6. **Runtime smoke-test — once it deploys (green CI is NOT proof it works).** Once gates are installed this **fires automatically in CI** (`runtime-smoke.yml`, on GitHub's `deployment_status` event — reads routes from `.shipit-gates/smoke.conf`), so you usually just read its result. To run it locally / on-demand when a preview-or-prod URL exists, hit the *live* artifact directly:
   ...
   **Browser-enforced behaviour needs a REAL browser, not a header assertion.** CSP, cache headers, cookie flags, and form/redirect navigation are enforced by the *browser*, not the server — a Node test can assert the response-header STRING but never its EFFECT. When a PR touches an auth/login/redirect/CSP page, the Playwright rung (`SHIPIT_SMOKE_UI` / `SHIPIT_SMOKE_E2E_CMD`) must LOAD that page and exercise the critical action (submit → assert the redirect fires; check the console for CSP violations). FocusBoard's OAuth login was blocked in prod by its own `form-action 'self'` CSP — present and "correct" as a string, but it polices the form's cross-origin 302; curl e2e and route tests don't enforce CSP, so all green. Header-string assertions are a regression pin, not proof. (Same gap class as the adapter-bypass 504/form-body bugs: the bug lives where the harness diverges from the real client.)
